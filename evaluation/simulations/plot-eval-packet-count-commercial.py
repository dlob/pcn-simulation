...
plotPath = os.path.abspath('eval-packet-count-commercial.pdf')
...
scaleF = 1.2
fig = plt.figure(figsize=(4.13 * scaleF, 3.3 * scaleF), tight_layout={'rect': [0, 0.09, 1, 1], 'w_pad': 4})
...
for scenarioName in scenarios:
	...
	for stdoutFile in stdoutFiles:
		...
		x_val = [e[0] for e in routerNetworkUsage]
		y_val = [e[1] for e in routerNetworkUsage]
		y_val = savgol_filter(y_val, 31, 3)
		t, c, k = interpolate.splrep(x_val, y_val)
		spline = interpolate.BSpline(t, c, k, extrapolate=False)
		xx = np.arange(0, 520, 20)
		xx[0] = 1
		axs[0, 0].plot(xx, spline(xx), lineTypes[algName], label=algNames[algName])
